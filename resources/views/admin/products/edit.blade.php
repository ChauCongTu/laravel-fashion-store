...
@section('content')
    <section class="p-5">
        <form action="{{ route('quan-ly-san-pham.update', ['quan_ly_san_pham' => $product]) }}" method="post" enctype="multipart/form-data">
            <div class="col-lg-12">
                <div class="card">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="card-title">
                            <h3 class="text-center title-2">CHỈNH SỬA SẢN PHẨM</h3>
                        </div>
                        <hr>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input name="summary" type="text" class="form-control" value="{{ $product->summary }}">
                            @error('summary')
                                <div class="text-danger">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="cat_id" class="form-control form-select-control">
                                @foreach ($categories as $item)
                                    <option value="{{ $item->id }}" {{ $product->cat_id == $item->id ? 'selected' : false }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="brand_id" class="form-control form-select-control">
                                @foreach ($brand as $item)
                                    <option value="{{ $item->id }}" {{ $product->brand_id == $item->id ? 'selected' : false }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
